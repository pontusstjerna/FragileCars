...
public class TapeBot implements GameObject {
    ...
    private void actOnState(States state) {
        ...
        switch (state) {
            ...
            case WEAK_RIGHT:
                ...
                break;
            case SHARPER_LEFT:
                turnDynamic(Dir.RIGHT, 10);
                break;
            case SHARPER_RIGHT:
                turnDynamic(Dir.LEFT, 10);
                break;
        }
    }
    ...
    private void followTape(ArrayList<BotPoint> tape) {
        ...
        if (car.getAcceleration() < speedLimit)
            car.accelerate();
    ...
    private void checkProgress() {
        if (state == States.STRAIGHT) {
            ...
            // System.out.println("tapeSize: " + mainTape.size() + " oldTapeLength: " +
            // oldTapeLength + " abs: " + Math.abs(mainTape.size() - oldTapeLength));
        }
    }
    ...
    private void lockLap() {
        if (car.getLaps() > 0) {
            ...
            followMode = true;
            passedGoalLine = true;
            // Try to go as fast as possible, then when dying, slow down until speed is
            // perfect
    ...
    private void slowDown() {
        ...
        if (speedLimit > 50) {
            speedLimit -= 50;
            System.out.println("Speed for " + car.getName() + ": " + speedLimit);
        } else { // Should never get here but if it does there is something seriously wrong with
                 // our lap
            followMode = false;
            // Reset everything
            mainTape.clear();
