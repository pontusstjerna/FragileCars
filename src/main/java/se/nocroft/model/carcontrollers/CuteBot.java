...
public class CuteBot implements GameObject {
    ...
    private void avoidTape() {
        ...
        if (onTape(leftStick.x, leftStick.y) && onTape(rightStick.x, rightStick.y)) { // If on tape, just keep driving
                                                                                      // in the direction we were to
                                                                                      // avoid
            setSpeed(50);

            // If both sticks are inside tape, decrease their length and see if still inside
            // and continue until one is outside
