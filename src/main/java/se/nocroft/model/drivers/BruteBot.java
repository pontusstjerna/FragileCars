    ...
    private Dir bestTurn(int x, int y, double heading) {
        if (walls.size() > 0) {
            ...
            try {
                ...
                // Try to turn right
                while (radsRight < Math.PI / 2
                        && (getClosestWallPoint(sxR, syR).distance(sxR, syR) < WALL_THRESHOLD + 1)
                ...
                /*
                 * IDEA!! If the ray never reaches max length (500), then turn to the dir where
                 * the longest ray was reached!!!
                 */
