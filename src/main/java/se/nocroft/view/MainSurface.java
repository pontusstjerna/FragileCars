...
public class MainSurface extends JPanel {
    ...
    private void paintCars(Graphics2D g) {
        for (int i = 0; i < track.getDrawableCars().length; i++) {
            ...
            // int x = (int)((track.getDrawableCars()[i].getX() -
            // track.getDrawableCars()[i].getWidth()/2)*scale()) + scaleX();
            // int y = (int)((track.getDrawableCars()[i].getY() -
            // track.getDrawableCars()[i].getHeight()/2)*scale());
            int x = (int) (track.getDrawableCars()[i].getX() * scale + scaleX());
            int y = (int) (track.getDrawableCars()[i].getY() * scale);

            double middleX = (track.getDrawableCars()[i].getX()
                    + ((track.getDrawableCars()[i].getWidth()) * Math.cos(heading) * Math.cos(heading)
                            + (track.getDrawableCars()[i].getHeight()) * Math.sin(heading) * Math.sin(heading)) / 2)
                    * scale;

            double middleY = (track.getDrawableCars()[i].getY()
                    + ((track.getDrawableCars()[i].getWidth()) * Math.pow(Math.sin(heading), 2)
                            + (track.getDrawableCars()[i].getHeight()) * Math.pow(Math.cos(heading), 2)) / 2)
                    * scale;
